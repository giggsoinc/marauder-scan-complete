...
| P2 | Local pre-commit hook scans staged content for `AKIA[A-Z0-9]{16}`, `ASIA…`, `AWSAccessKeyId=AKIA`, `AWS_SECRET_ACCESS_KEY=…` (40-char shape), and the rendered installer file paths | [`ghost-ai-scanner/scripts/git-hooks/pre-commit`](ghost-ai-scanner/scripts/git-hooks/pre-commit) | Install: `bash ghost-ai-scanner/scripts/git-hooks/install.sh` (each developer, once after clone). |
